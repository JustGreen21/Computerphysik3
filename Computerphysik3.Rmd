...
\end{equation}
Hierbei entspricht S der Zahl der noch nicht Infektierten, I der aktuell Kranken, R der Genesenen und V der Geimpften. Wir werden im Verlauf der Simulationen die Geimpftenzahl mit der Anzahl der Personen mit Zweitimpfung gleichsetzen, da ab diesem Punkt der vollständige Schutz eintritt. Desweiteren ist N die Gesammtbevölkerung, $R_0$ entspricht der Basisreproduktionszahl, $\alpha$ der inversen Infektiositätszeit , $\beta = R_0 * \alpha$, $\Gamma$ die Impfungsrate und $\delta$ die Rate mit der Geimpfte erkranken.
...
Vorerst wählen wir die Schrittweit h = 1, da dies direkt den Tagen entspricht.
Dabei bietet uns das Rung-Kutter Verfahren nummerische Stabilität und weist mit einem Verfahrensfehler von $\mathcal{O}(h^2)$ einen kleineren Fehler als das Eulerverfahren auf.
...
Betrachten wir nun den Verlauf ohne Gegenmaßnahmen, also $\Gamma = \delta = 0$ und den SARS_CoV-2 spezifischen Daten $R_0 \approx 2.9$ und $\alpha = \frac{1}{7 \, \mathrm{Tage}}$, so ergibt sich Abb. \ref{fig:grundEntwicklung}.
```{r, echo=FALSE}
#Anfangsbedingungen
N = 8.3e7
S = N - 5000
...
Nun wollen wir noch prüfen, ob sich die Verläufe nach der Herdenimunität verhalten. 
Diese ist gegeben durch: 
\begin{equation}
  \frac{S}{N} = \frac{1}{R_0}
\end{equation}
Dazu tragen wir am Ende der Pandemie (d.h.$I(t) < 1$) das Verhältnis der noch nicht Infizierten $S(t)$ gegenüber der Gesammtbevölkerung $N$ in Abhänigkeit von $R_0$ auf. Zum Vergleich betrachten wir auch den nach der Herdinimunität erwarteten Kurvenverlauf. 
...
legend(0.05,1,legend=c("Simulation", "Erwartung nach Herdenimunität"), col=c("blue","gray"), lty=1:1)

```

In  Abb. \ref{fig:Herde} ist zu erkennen, dass der von uns simulierte Verlauf, deutlich von der Herdenimunität abweicht.
Dies sollte an der sehr schnellen Verbreitung der Kankheit für größe $R_0$ Werte liegen. Nach Abb. \ref{fig:EntwicklungVarR} ist zu erkennen, dass nach wenigen Wochen, die Krankheit wieder abklingt, dies liegt jedoch daran, dass alle gesunden Menschen schon angesteckt wurden und damit keinen neuen Wirte mehr vorhanden sind.
Diese kleine Menge an "gesunden" Menschen nach der Pandiemie erklärt die größe Abweichung zur Herdenimunität, bei der größere Teile der Bevölkerung nach der Pandemie nie erkrankt waren.
Es ist jedoch zu erwähnen, dass die Simualtion ihre Schwächen hat, da durch keine Gegenmaßnahmen das Infektionswesen wärend der ganzen Zeit für größe $R_0$ Werte auch groß bleibt. Unter Einbeziehung von Gegenmaßnahmen sollte man sich der Erwartung nach Herdenimunität nähern.    
...
Desweiteren wurde $\gamma$ und $\mathrm{N}_0$ als Treppenfunktion mit den Werten
...
Um den Verlauf der Pandemie möglich realitätsnah zu Simulieren, wurden die $R_0$ werte so angepasst, dass die aktiven Fälle der Simulation zum jeweiliegen Zeitpunkt (der Daten) mit den Daten des RKI in näherung übereinstimmt. Abweichung von $\Delta I = 300$ wurden als vernachlässigbar klein angenommen, da $I \gg \Delta I$ ist im betrachtetem Bereich.
Um den Pandemieverlauf besser betrachten zu können haben wir im folgenden Diagramm nicht nur die 365 Tage vom Jahr 2021 aufgetragen, sondern großteile der Jahres 2022 mit einbezogen.
...
```{r, echo=FALSE, fig.cap="\\label{fig:wahrerVerlauf} $R_0$ abhänginge Herdenimmunität"}
...
legend(350,3e5,legend=c("realer Verlauf für h = 1","realer Verlauf für h = 2","realer Verlauf für h = 0,5","realer Verlauf für h = 0,01","alternativer Verlauf"), col = c("dark blue","green","black","gray","red"), lty=1:1)
```
...
Falls das $h$ jedoch noch größer gewählt werden würde, wäre der Verlauf nicht mehr ümbedingt representabel, da die genutzten Daten einen gewissen Abstand (weniger als 40 Tage) haben und die benutzte Datenmenge an sich schon recht knapp ist.
Dies geringe Abhänigkeit von $h$ zeigt jedoch, dass das genutzt Runge-Kutter Verfahren ein stabieles Verfahren ist, welches gut Werte produziert.
...
Es ist auch auffällig das der hypothetische Verlauf für die ersten ca. 70 Tage eine in Näherung gleichen Verlauf hat wie der reale Verlauf.
Jedoch hat der reale Verlauf ab ca. Tag 70 bis Tag 110 einen großen anstieg der Infizieten und fällt dann wieder ab. Erst stärker und ab ca. Tag 150 wieder schwächer.
Am Ende des Jahres bei Tag 362 liegen beide Verläufe wieder sehr nah bei einander. An dies auch der Punkt ist an dem die Pandemie nach der Simulation als Angeklungen bezeichnet werden kann, da der weite Verlauf keine verschlimmerung aufweist (dies ist jedoch sehr hypothetisch da eine solche Vorraussage auf eingeschränkten annahmen beruht und versucht recht weite Zeitraume zu deuten).
Der in realen Verlauf auftauchende Peak lässt sich durch frühzeitigen Lockerungen Anfang März erklären, was  zu einem rapieden Anstieg von infizierten führte (Lockerungsmaßnahmen \cite{lockerungenmaerz}).


# Fazit

Mit hilfe des Runge-Kutta 2 Verfahren konnten wir den Pandemieverlauf, mit dem DGS in guter Näherung simulieren.
Dabei stellten wir fest, dass große $R_0$ wie auch zu erwarten  einen schnellen Anstieg der Infizierten zur folge hat. 
Es war weitergehend zu erkennen, dass sich durch dies schnelle Verbreitung eine diskrepanz zwichen der Simulation und der Herdenimunität ergibt.
Bei der Konstrucktion des realen Verlauf konnte man, durch die konstantheit des Verlaufs unter Variation der Schrittweite eine stärke der Runge-Kutta Verfahrens aufzeigen.
Der zusätzliche hypothetische Verlauf wäre eine alternative zum realenverlauf gewessen, hätte aber weiter soziale Einschränkungen zu folge gehabt.
Wenn man keinen verschlimmerungen zu erwarten hat kann man mit einem ungefährem Ende der Pandemie spätestens Ende des Jahres 2021 rechnen.
