...
//[[Rcpp::export]]
Rcpp::List durchlauf(const int schritte, const double h,
  ...
  // Array der Werte:
    Rcpp::NumericVector xValue(schritte);
    Rcpp::NumericVector SValue(schritte);
    Rcpp::NumericVector IValue(schritte);
    Rcpp::NumericVector RValue(schritte);
    Rcpp::NumericVector VValue(schritte);
  // Quelltext
  ...
  Parameter parameter = {.alpha = alpha, .beta = beta, .delta = delta, .N = N, .Gamma = Gamma};
    ...
    for (int i = 0; i < schritte; i++){
      ...
      if(status.I < 1 && i>10){
          ...
          return List::create(Named("x") = xValue, Named("S") = SValue, Named("I") = IValue,
                              Named("R") =  RValue, Named("V") = VValue);
      }
    }
  // Rückgabe für eine grafische Wiedergabe
...
```{r, echo=FALSE}
plot(schutzlos$x, schutzlos$S, "l", xlab = "Tage", ylab = "Gesamtfehler", col = "dark blue")
lines(schutzlos$x, schutzlos$I, "l", col = "green")
