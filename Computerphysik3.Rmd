...
Dabei bietet uns das Rung-Kutter Verfahren nummerische Stabilität und weist mit einem Verfahrensfehler von $\mathcal{O}(h^2)$ einen kleineren Fehler als das Eulerverfahren aus.
...
Für den ausgebrochen einer Krankheit in Deutschland sind die Anfangswerte gegeben wie folgt.\\
...
```
Nun wollen wir noch Prüfen ob sich die Verfläufe nach der Herdenimunität verhalten. 
Diese ist gegeben durch: 
\begin{equation}
  \frac{S}{N} = \frac{1}{R_0}
\end{equation}
Dazu tragen wir die am ende der Pandemie (d.h.$I(t) < 0$), das Verhältnis der noch nicht Infizierten $S(t)$ gegenüber der Gegsamtbevölkerung $N$ auf in abhänigkeit von $R_0$. Zum Vergleich betrachten wir auch den nach der Herdinimunität erwarteten Verlauf. 
...
In  Abb. \ref{fig:Herde} ist zu erkennen, dass der von uns simulierte Verlauf, der zu erwarten ist, deutlich vom der Herdenimunität abweicht.
Dies sollte an der sehr schnellen Verbreitung der Kankheit für größe $R_0$ Werte liegen. Nach Abb. \ref{fig:EntwicklungVarR0} ist zu erkennen, dass nach wenigen Wochen, die Krankheit wieder abklingt, dies liegt jedoch daran, dass alle gesunden Menschen schon angesteckt würden und damit keinen neuen Wirte mehr vorhanden sind.
Diese keine Menge an "gesunden" Menschen nach der Pandiemie erklärt die größe abweichung zur Herdenimunität, bei der größere Teile der Bevölkerung nach der Pandemie nie erkannt waren.
Es ist jedoch zu erwähnen, das die Simualtion ihre schwächen hat, da durch keine Gegenmaßnahmen das Infektionswessen wärend des ganzen für größe $R_0$ Werte auch groß bleibt. Unter Einbeziehung von Gegenmaßnahmen sollte man sich der Erwartung nach Herdenimunität nähern.    
...
\begin{table}[h!]
    ...
    \end{tabular}
    \caption{RKI Daten zur Covid-19 Entwicklung von 3.1.21- 1.6.21 , \cite{janunar} bis \cite{juni}}
    \label{tab:data}
\end{table}
...
legend(350,3e5,legend=c("realer Verlauf","alternativer Verlauf"), col = c("dark blue","red"), lty=1:1)
```

# Fazit

Machen wir sowas überhaupt?
